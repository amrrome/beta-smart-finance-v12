...
<body>
    ...
    <script type="text/babel">
        ...
        class SmartPaymentEngine {
            ...
            // ═══ FIND INSTALLMENT BY XNPV (pure date-based binary search) ═══
            // Always uses actual calendar dates — works with any start date or frequency
            findInstallmentByXNPV(targetPV, dates) {
            ...
            calculateEqualInstallments(params) {
                ...
                if (distributeUnitValue) {
                    ...
                    // ═══ حساب PV-based (XNPV بالتواريخ الفعلية) ═══
                    ...
                    // XNPV binary search بالتواريخ الفعلية
                    installmentAmount = this.findInstallmentByXNPV(remainingPV, quarterlyDates);
            ...
            calculateDecreasingPayments(params) {
                ...
                const baseInstallment = this.findBaseInstallmentDecreasing(remainingPV, quarterlyDates, decreaseRate / 100, frequency);
                ...
                const freqLabel = frequency === 4 ? 'ربع سنوي' : frequency === 2 ? 'نصف سنوي' : 'سنوي';
                quarterlyDates.forEach((date, index) => {
                    const yearIndex = Math.floor(index / frequency);
                    const periodInYear = (index % frequency) + 1;
                    const amount = baseInstallment * Math.pow(1 - decreaseRate / 100, yearIndex);
                    const pv = this.calculatePVWithDate(amount, date);
                    schedule.push({
                        id: paymentId++,
                        type: `قسط متناقص - سنة ${yearIndex + 1} / ${freqLabel} ${periodInYear}`,
                        date: this.formatDateLocal(date),
                        amount,
                        pv,
                        discount: amount - pv
            ...
            findBaseInstallmentDecreasing(targetPV, dates, decreaseRate, frequency) {
                let low = 0, high = targetPV * 2, tolerance = 0.01;
                const maxIterations = 200;
                let iterations = 0;
                ...
                while (high - low > tolerance && iterations < maxIterations) {
                    ...
                    let calculatedPV = 0;
                    for (let i = 0; i < dates.length; i++) {
                        const yearIndex = Math.floor(i / frequency);
                        const amount = mid * Math.pow(1 - decreaseRate, yearIndex);
            ...
            calculateGraduatedPayments(params) {
                ...
                const baseInstallment = this.findBaseInstallmentGraduated(remainingPV, quarterlyDates, growthRate / 100, frequency);
                ...
                const freqLabelG = frequency === 4 ? 'ربع سنوي' : frequency === 2 ? 'نصف سنوي' : 'سنوي';
                quarterlyDates.forEach((date, index) => {
                    const yearIndex = Math.floor(index / frequency);
                    const periodInYear = (index % frequency) + 1;
                    const amount = baseInstallment * Math.pow(1 + growthRate / 100, yearIndex);
                    const pv = this.calculatePVWithDate(amount, date);
                    schedule.push({
                        id: paymentId++,
                        type: `قسط متزايد - سنة ${yearIndex + 1} / ${freqLabelG} ${periodInYear}`,
                        date: this.formatDateLocal(date),
                        amount,
                        pv,
                        discount: amount - pv
            ...
            findBaseInstallmentGraduated(targetPV, dates, growthRate, frequency) {
                let low = 0, high = targetPV * 2, tolerance = 0.01;
                const maxIterations = 200;
                let iterations = 0;
                ...
                while (high - low > tolerance && iterations < maxIterations) {
                    ...
                    for (let i = 0; i < dates.length; i++) {
                        const yearIndex = Math.floor(i / frequency);
                        const amount = mid * Math.pow(1 + growthRate, yearIndex);
                        calculatedPV += this.calculatePVWithDate(amount, dates[i]);
                    }
            ...
            calculateCustomDistribution(params) {
                ...
                if (quarterlyYears > 0) {
                    ...
                    const installmentAmount = this.findInstallmentByXNPV(remainingPV, installmentDates);
            ...
            verifyScheduleXNPV(schedule) {
                // التحقق من PV المحسوب في الجدول
                // الدفعات المعروفة: PV بالتواريخ الفعلية
                // الأقساط: PV بالمعدل الدوري (periodic)
                // كلاهما محسوب بالفعل في schedule.pv
            ...
            const calculateResults = () => {
                ...
                try {
                    ...
                    if (project.baseSystem) {
                        ...
                        baseResult.verification = baseEngine.verifyScheduleXNPV(baseFinalSchedule); // Base system always quarterly
                ...
                <div className="min-h-screen p-4 md:p-8 bg-gray-100">
                    <div className="max-w-7xl mx-auto">
                        ...
                        <div className="text-center text-gray-600 text-sm mt-8 pb-8 print-hidden">
                            ...
                            <p className="mt-1">BETA SMART FINANCE | نظام التقسيط المرن - DR. AMR ALTAMAWI</p>
